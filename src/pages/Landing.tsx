...
function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-[#151921] border-b-2 border-[#3F4753]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            <a href="#features" className="text-sm text-[#9CA3AF] hover:text-white font-medium transition-colors">
              Fonctionnalités
            </a>
            <a href="#how-it-works" className="text-sm text-[#9CA3AF] hover:text-white font-medium transition-colors">
              Comment ça marche
            </a>
            <a href="#proof" className="text-sm text-[#9CA3AF] hover:text-white font-medium transition-colors">
              Exemples
            </a>
          </div>
          ...
          {/* Mobile hamburger */}
          <button
            className="md:hidden p-2 hover:bg-[#1C2128] transition-colors border border-[#2D3541]"
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden bg-[#151921] border-t-2 border-[#2D3541] px-4 py-4 space-y-3 animate-fade-in">
          {['Fonctionnalités', 'Comment ça marche', 'Exemples'].map(label => (
            <a
              key={label}
              href={`#${label.toLowerCase().replace(' ', '-')}`}
              className="block py-2 text-[#9CA3AF] hover:text-white font-medium border-b border-[#2D3541]"
...
function Hero() {
  ...
  return (
    <section
      className="relative min-h-screen flex items-center overflow-hidden bg-[#0A0E14]"
    >
      {/* Background grid */}
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-24 lg:py-32">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-8 animate-slide-up">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#151921] border-2 border-[#3F4753]">
              <span className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse-slow" />
              <span className="text-[#9CA3AF] text-sm font-mono">IA de pointe · Analyse en ~3 minutes</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 text-[#9CA3AF] hover:text-white font-medium transition-colors"
              >
            ...
            <div className="flex items-center gap-6 pt-2">
              <div className="flex -space-x-1">
                {['🧑‍💻', '👩‍💼', '🧑‍🚀', '👩‍🔬'].map((emoji, i) => (
                  <div
                    key={i}
                    className="w-8 h-8 bg-[#1C2128] border-2 border-[#3F4753] flex items-center justify-center text-sm"
                  >
                    {emoji}
                  </div>
                ))}
          ...
          <div className="hidden lg:flex justify-center animate-scale-in animation-delay-300">
            <div className="relative">
              {/* Main card */}
              <div className="bg-[#1C2128] border-2 border-[#3F4753] p-8 w-80" style={{ boxShadow: '8px 8px 0px rgba(230,57,70,0.2)' }}>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    { label: 'Concurrence',   score: 40, color: '#F59E0B' },
                  ].map(item => (
                    <div key={item.label} className="flex items-center gap-3">
                      <span className="text-xs text-[#6B7280] w-24 text-right flex-shrink-0 font-mono">{item.label}</span>
                      <div className="flex-1 h-1.5 bg-[#2D3541] overflow-hidden">
                        <div
                          className="h-full"
                          style={{ width: `${item.score}%`, backgroundColor: item.color }}
                        />
                      </div>
                      <span className="text-xs font-bold text-white w-7 font-mono">{item.score}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="absolute -bottom-4 -left-4 bg-[#1C2128] text-emerald-400 text-xs font-bold font-mono px-3 py-1.5 border-2 border-emerald-700">
                ✓ Bon potentiel
              </div>
            </div>
          </div>
...
function Features() {
  ...
  return (
    <section id="features" className="py-24 bg-[#151921]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <span className="badge bg-primary-900/30 text-primary-400 border border-primary-800 mb-4 font-mono text-xs uppercase tracking-wider">
            Fonctionnalités
          </span>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {features.map((f, i) => (
            ...
            >
              <div className="w-16 h-16 bg-[#1C2128] border-2 border-[#3F4753] flex items-center justify-center mx-auto">
                {f.icon}
              </div>
              <h3 className="text-xl font-bold text-white font-display">{f.title}</h3>
              <p className="text-[#9CA3AF] leading-relaxed">{f.description}</p>
              <span className="badge bg-[#1C2128] text-[#6B7280] border border-[#2D3541] text-xs font-mono mx-auto">
                {f.detail}
              </span>
            </div>
          ))}
...
function HowItWorks() {
  ...
  return (
    <section id="how-it-works" className="py-24 bg-[#0A0E14]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <span className="badge bg-secondary-900/30 text-secondary-500 border border-secondary-800 mb-4 font-mono text-xs uppercase tracking-wider">
            Comment ça marche
          </span>
        ...
        <div className="grid md:grid-cols-3 gap-8 relative">
          {/* Connector line */}
          <div className="hidden md:block absolute top-10 left-[calc(16.67%+2rem)] right-[calc(16.67%+2rem)] h-0.5 bg-[#2D3541]" aria-hidden />

          {steps.map((step, i) => (
            <div key={i} className="relative flex flex-col items-center text-center gap-4">
              <div className="w-20 h-20 bg-[#151921] border-2 border-[#3F4753] flex flex-col items-center justify-center gap-1 relative z-10" style={{ boxShadow: '4px 4px 0px rgba(230,57,70,0.15)' }}>
              ...
              <div>
                <h3 className="font-bold text-white text-lg font-display">{step.title}</h3>
                <p className="text-[#9CA3AF] text-sm leading-relaxed mt-2">{step.desc}</p>
                <span className="inline-flex items-center gap-1 mt-3 text-xs font-medium text-[#4B5563] font-mono">
                  <Clock size={12} />
                  {step.duration}
                </span>
              </div>
...
function SocialProof() {
  ...
  return (
    <section id="proof" className="py-24 bg-[#151921]">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-20">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {exampleScores.map((item, i) => (
              <div key={i} className="card p-6 text-center space-y-4">
                <span className="badge bg-[#1C2128] text-[#9CA3AF] border border-[#2D3541] text-xs font-mono mx-auto uppercase">{item.industry}</span>
                <div className="mx-auto">
                  <ScoreGauge score={item.score} size="sm" animate={false} showLabel={false} />
                </div>
                <div>
                  <p className="font-semibold text-white font-display text-sm">{item.name}</p>
        ...
        <div>
          <div className="text-center mb-12">
            <span className="badge bg-emerald-900/30 text-emerald-400 border border-emerald-800 mb-4 font-mono text-xs uppercase tracking-wider">
              Témoignages
            </span>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <div key={i} className="card p-6 space-y-4">
                ...
                <div className="flex items-center gap-3 pt-2 border-t-2 border-[#2D3541]">
                  <div className="w-8 h-8 bg-primary-900/30 border border-primary-800 flex items-center justify-center text-xs font-bold text-primary-400 font-mono">
                    {t.author.charAt(0)}
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-white font-display">{t.author}</p>
                    <p className="text-xs text-[#6B7280] font-mono">{t.role}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* Final CTA — flat red, no gradient */}
        <div className="bg-primary-500 border-2 border-primary-700" style={{ boxShadow: '8px 8px 0px rgba(0,0,0,0.4)' }}>
          <div className="p-12 text-center space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button
                onClick={() => navigate('/submit')}
                className="group inline-flex items-center gap-3 px-10 py-4 bg-white text-[#0A0E14] text-lg font-bold transition-all duration-200 font-display uppercase tracking-[0.05em]"
...
function Footer() {
  return (
    <footer className="bg-[#0A0E14] border-t-2 border-[#2D3541] text-[#6B7280] py-12">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          ...
          <div className="flex gap-8 text-sm">
            <div className="space-y-2">
              <p className="font-bold text-[#9CA3AF] text-xs uppercase tracking-wider font-mono">Produit</p>
            ...
            <div className="space-y-2">
              <p className="font-bold text-[#9CA3AF] text-xs uppercase tracking-wider font-mono">Légal</p>
              <ul className="space-y-1.5">
        ...
        <div className="mt-8 pt-8 border-t-2 border-[#2D3541] flex flex-col sm:flex-row items-center justify-between gap-4 text-xs font-mono">
