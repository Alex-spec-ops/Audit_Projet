    ...
    <div className={`card p-4 border-l-4 ${borderColor} group relative`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold uppercase tracking-wider text-[#4B5563] font-mono">{label}</span>
          {confidence && (
        ...
        {editable && !editing && (
          <button
            ...
            className="opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-[#1C2128] border border-[#2D3541]"
            title="Modifier"
          >
      ...
      {editing ? (
        <div className="space-y-2">
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => setEditing(false)} className="text-xs text-[#6B7280] hover:text-white px-3 py-1 border border-[#3F4753] hover:bg-[#1C2128] font-mono">
              Annuler
            </button>
        ...
        <div>
          {isEmpty ? (
            <p className="text-sm text-[#4B5563] italic font-mono">Non détecté — à préciser</p>
          ) : isList ? (
            ...
            <p className="text-sm text-[#D1D5DB] leading-relaxed">{value as string}</p>
          )}
        </div>
      )}
    </div>
...
const CATEGORY_STYLES: Record<ClarificationQuestion['category'], { border: string; bg: string }> = {
  cible:         { border: 'border-l-4 border-blue-700',    bg: 'bg-blue-900/10' },
  modele:        { border: 'border-l-4 border-emerald-700', bg: 'bg-emerald-900/10' },
  fonctionnalites:{ border: 'border-l-4 border-purple-700', bg: 'bg-purple-900/10' },
  problematique: { border: 'border-l-4 border-secondary-700', bg: 'bg-secondary-900/10' },
  valeur:        { border: 'border-l-4 border-primary-700', bg: 'bg-primary-900/10' },
};

const CATEGORY_ICONS: Record<ClarificationQuestion['category'], React.ReactNode> = {
  cible:          <Users size={16} className="text-blue-400" />,
  modele:         <DollarSign size={16} className="text-emerald-400" />,
  fonctionnalites:<Zap size={16} className="text-purple-400" />,
  problematique:  <AlertCircle size={16} className="text-secondary-500" />,
...
function QuestionCard({ q, value, onChange }: QuestionCardProps) {
  ...
  return (
    <div className={`border-2 border-[#2D3541] p-4 ${styles.border} ${styles.bg}`}>
      <div className="flex items-start gap-2 mb-3">
        {CATEGORY_ICONS[q.category]}
        <p className="text-sm font-semibold text-white leading-relaxed font-display">{q.question}</p>
      </div>
      <textarea
        value={value}
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] space-y-8 animate-fade-in">
      <div className="relative w-24 h-24">
        <div className="absolute inset-0 rounded-full border-2 border-[#2D3541] border-t-primary-500 animate-spin" style={{ animationDuration: '1.2s' }} />
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-4xl transition-all duration-500" key={step}>{steps[step].emoji}</span>
        </div>
      </div>
      <div className="text-center space-y-2">
        <p className="text-[#4B5563] text-xs uppercase tracking-widest font-mono font-medium">Analyse de</p>
        <h2 className="text-xl font-black text-white font-display">"{projectName}"</h2>
      </div>
          ...
          <div key={i} className={`flex items-center gap-3 transition-all duration-400 ${i <= step ? 'opacity-100' : 'opacity-25'}`}>
            <div className={`w-5 h-5 flex-shrink-0 flex items-center justify-center transition-all duration-300 border-2
              ${i < step
                ...
                : i === step
                ? 'bg-primary-900/30 border-primary-600 animate-pulse'
                : 'bg-[#1C2128] border-[#2D3541]'}`}>
              {i < step && <CheckCircle size={10} className="text-emerald-400" />}
            </div>
            <span className={`text-sm font-mono ${i === step ? 'text-white font-semibold' : i < step ? 'text-emerald-400' : 'text-[#4B5563]'}`}>{s.label}</span>
          </div>
        ))}
      </div>
    </div>
...
function ConfidenceBar({ value }: { value: number }) {
  ...
  const textColor = value >= 70 ? 'text-emerald-400' : value >= 45 ? 'text-amber-400' : 'text-primary-400';

  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 h-2 bg-[#2D3541] overflow-hidden">
...
export default function Extract() {
    ...
    return (
      <div className="min-h-screen bg-[#0A0E14] flex items-center justify-center">
        <div className="text-center space-y-4">
          <p className="text-[#6B7280] font-mono">Aucune donnée de projet trouvée.</p>
  ...
  return (
    <div className="min-h-screen bg-[#0A0E14]">
      {/* Header */}
      <header className="bg-[#151921] border-b-2 border-[#3F4753] sticky top-0 z-40">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
          <Link to="/submit" className="flex items-center gap-2 text-[#6B7280] hover:text-white transition-colors">
            <ArrowLeft size={18} />
            <div className="w-7 h-7 bg-primary-500 flex items-center justify-center text-white text-xs font-black font-mono">◈</div>
            <span className="font-bold text-white hidden sm:block font-display">VeritAudit.ai</span>
          </Link>
          {/* Step indicator */}
              ...
              <div key={s.n} className="flex items-center gap-2">
                <div className={`w-7 h-7 flex items-center justify-center text-xs font-bold font-mono border-2 transition-all
                  ${s.done
                    ...
                    : 'bg-[#1C2128] border-[#2D3541] text-[#4B5563]'}`}>
                  {s.done ? '✓' : s.n}
                </div>
                {i < arr.length - 1 && <div className={`h-0.5 w-5 ${s.done ? 'bg-emerald-800' : 'bg-[#2D3541]'}`} />}
              </div>
            ))}
            <span className="ml-2 text-xs text-[#6B7280] hidden sm:block font-mono">Extraction IA</span>
          </div>
        </div>
      </header>
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h1 className="text-2xl font-black text-white font-display">Extraction IA terminée</h1>
              </div>
            ...
            <div className="space-y-4">
              <h2 className="text-xs font-bold text-[#4B5563] uppercase tracking-wider font-mono">Données extraites</h2>
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-sm text-primary-500 hover:text-primary-400 font-medium font-mono"
              >
            ...
            {state.status === 'clarifying' && state.questions.length > 0 && (
              <div className="space-y-4 p-5 bg-secondary-900/10 border-2 border-secondary-800 animate-fade-in">
                <div className="flex items-start gap-3">
                  <HelpCircle size={20} className="text-secondary-500 flex-shrink-0 mt-0.5" />
                  <div>
                    <h2 className="font-bold text-secondary-400 text-base font-display">
                    ...
                    <p className="text-[#9CA3AF] text-sm mt-0.5 font-mono">
            ...
            {/* Action buttons */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t-2 border-[#2D3541]">
              <button
                onClick={() => navigate('/submit')}
                className="flex items-center gap-2 text-sm text-[#6B7280] hover:text-white font-medium font-mono transition-colors"
              >
                <RefreshCw size={15} />
            ...
            {state.status === 'clarifying' && (
              <p className="text-center text-xs text-[#4B5563] font-mono">
