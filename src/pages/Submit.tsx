          ...
          <div
            className={`w-7 h-7 flex items-center justify-center text-xs font-bold font-mono transition-all duration-300
              ${s === step
                ...
                : s < step
                ? 'bg-[#1C2128] text-primary-400 border-2 border-primary-800'
                : 'bg-[#1C2128] text-[#4B5563] border-2 border-[#2D3541]'}`}
          >
            {s}
          </div>
          {s < 4 && <div className={`h-0.5 w-5 transition-colors duration-300 ${s < step ? 'bg-primary-700' : 'bg-[#2D3541]'}`} />}
        </div>
      ))}
      <span className="ml-2 text-[#6B7280] text-xs hidden sm:block font-mono">{labels[step - 1]}</span>
    </div>
  );
}
...
function FileBadge({ file, onRemove }: FileBadgeProps) {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-primary-900/20 border-2 border-primary-800">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-[#6B7280] font-mono">{sizeMB} MB</p>
      </div>
      <button
        onClick={onRemove}
        className="p-1 hover:bg-primary-900/40 transition-colors"
  ...
  return (
    <div className="min-h-screen bg-[#0A0E14]">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-10 space-y-8 animate-fade-in">
        {/* Heading */}
        <div>
          <h1 className="text-3xl font-black text-white font-display">Décrivez votre projet</h1>
        ...
        {/* Mode toggle */}
        <div className="flex gap-0 p-1 bg-[#1C2128] border-2 border-[#2D3541] w-fit">
        ...
        {mode === 'text' && (
          <div className="space-y-2 animate-fade-in">
            <div className="relative">
              ...
              />
              <div className="absolute bottom-3 right-3 text-xs text-[#4B5563] font-mono bg-[#151921] px-2 py-1 border border-[#2D3541]">
                {description.length}/{MAX_CHARS}
              </div>
            </div>

            {/* Writing tips */}
            <div className="p-4 bg-secondary-900/20 border-2 border-secondary-800 flex gap-3">
              <Lightbulb size={18} className="text-secondary-500 flex-shrink-0 mt-0.5" />
              <div className="text-sm text-secondary-400 space-y-1">
                <p className="font-semibold font-display">Pour un audit optimal, incluez :</p>
                <ul className="space-y-0.5 text-[#9CA3AF]">
                  {[
                    ...
                    <li key={i} className="flex items-start gap-1.5 font-mono text-xs">
                      <span className="mt-0.5 text-secondary-600">▸</span>
                      {tip}
                    </li>
                  ))}
                </ul>
        ...
        {mode === 'file' && (
          <div className="space-y-4 animate-fade-in">
            <div
              ...
              className={`relative border-2 border-dashed p-12 text-center cursor-pointer transition-all duration-200
                ${dragging
                  ...
                  : 'border-[#3F4753] hover:border-primary-700 hover:bg-primary-900/10'}`}
              role="button"
            ...
            {error && (
              <div className="p-3 bg-primary-900/20 border-2 border-primary-800 text-sm text-primary-400 flex items-start gap-2 font-mono">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label htmlFor="project-name" className="block text-sm font-semibold text-[#9CA3AF] mb-1.5 font-mono uppercase tracking-wider">
              Nom du projet <span className="text-[#4B5563] font-normal normal-case">(optionnel)</span>
            </label>
            <input
              id="project-name"
              type="text"
          ...
          <div>
            <label htmlFor="industry" className="block text-sm font-semibold text-[#9CA3AF] mb-1.5 font-mono uppercase tracking-wider">
              Secteur d'activité <span className="text-[#4B5563] font-normal normal-case">(optionnel)</span>
            </label>
            <div className="relative">
              <select
                id="industry"
              ...
              >
                <option value="" className="bg-[#1C2128]">Sélectionner un secteur...</option>
                {INDUSTRIES.map(ind => (
                  <option key={ind} value={ind} className="bg-[#1C2128]">{ind}</option>
                ))}
              </select>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pt-4 border-t-2 border-[#2D3541]">
          <div className="flex items-center gap-2 text-sm text-[#6B7280] font-mono">
            <Clock size={16} className="text-[#4B5563]" />
            <span>Analyse en <strong className="text-white">~2-3 minutes</strong> · Gratuit · Sans inscription</span>
          </div>
        ...
        {!canSubmit && (
          <p className="text-xs text-center text-[#4B5563] font-mono">
            {mode === 'text'
